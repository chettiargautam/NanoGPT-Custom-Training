...
def get_batch(data: torch.Tensor, split: str) -> tuple:
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, indices: torch.Tensor, targets: torch.Tensor = None):
        logits = self.token_embedding(indices)  # (B, T, C)
        loss = None
...
def train_model(model, train_data, val_data, model_path="bigram_model.pth"):
    ...
    for iteration in progress_bar:
        ...
        x_batch, y_batch = get_batch(train_data, 'train')
...
def main():
    parser = argparse.ArgumentParser(description="Bigram Language Model")
    ...
    if args.mode == 'train':
        ...
        model.load_state_dict(torch.load('bigram_model.pth'))  # Load trained model
